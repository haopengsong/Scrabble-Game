...
void Board::doPlace()
{
    ...
    //find the blank tiles and erase it
    ...
    //place start
    //check if game start at the star square
    ...
    {
        //set start identifier to false
        vb->set_starvirtualBoard(false);

        //vector to store the location of each square in word each letter
        ...
        if(!vb->boardobserver(vs_) && c_x!=7 && c_y!= 7)
        {
         ...
         //check_tiles_two : make sure the player has those tiles
        //if user has those tiles
